...
class Log(ABC):  # A classe que possuir um método abstrado deve herdar a função de abstração da classe ABC
    ...
    # Método Abstrato não tem sua execução partida da classe da qual ela pertence, a classe abstrata precisa ser implementada por
    # outra classe filha, usando os mesmo padrão de contruçao do método da classe pai
    def _log(self, msg):
        ...
...
class AbstractFoo(ABC):  # Classe pai que herda métodos e decoradores da classe ABC responsável pela criação de métodos abstratos
    def __init__(self, name):  # Método inicializador
        self._name = None  # Criamos primeiro o atributo _name que é o atributo receptor do valor a ser setado na property
        self.name = name  # O setter passar o valor para a property name para que o valor possa ser retornado

    # Criação de uma property abstrata, para que a property possa ser abstrata o decorator @abstractmethod precisa estar o mais
    # perto possivel da assinatura do method
    @property
    @abstractmethod
    def name(self):
        ...

    # Por haver um property abstrato, precisamos ter uma assinatura de um setter para passar o valor, que é definito tanto na
    # classe pai quanto na classe filha
    @name.setter
    # @abstractmethod O setter também pode ser abstrato e para isso passamos o decorator da mesma forma que na property e para
    # chama-lo nma classe filha usamos o decorator da seguinte forma @NomeDaClassePai.nomedometodo.setter
